...
DB_CONFIG = Path(os.environ.get("LSCC_DB", "lscc.db"))
DB_PATH = str(DB_CONFIG if DB_CONFIG.is_absolute() else BASE_DIR / DB_CONFIG)
ADMIN_INITIAL_PASSWORD = os.environ.get("ADMIN_INITIAL_PASSWORD")


def crear_base_de_datos():
    if not ADMIN_INITIAL_PASSWORD:
        raise SystemExit("Falta ADMIN_INITIAL_PASSWORD en dashboard/.env")
    ...
    # ----------------------------------------------------------
    # 8. USUARIOS — Login del dashboard + sesión única
    # ----------------------------------------------------------
    cur.execute("""
